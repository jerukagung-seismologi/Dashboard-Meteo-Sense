import { 
  LayoutDashboard, 
  Network, 
  FileText, 
  Database, 
  Earth, 
  Sprout, 
  ChartNoAxesCombined, 
  User,
  UsersRound,
  CloudRain,
  TrendingUp,
  Compass,
  Settings2,
  Globe,
  GitCompare,
  Wind,
  Radio,
  SlidersHorizontal,
  Archive,
  LucideIcon
} from "lucide-react"

export interface NavigationItem {
  name: string
  href: string
  icon: LucideIcon
  badge?: string
  requiredRole?: string[]
  roles?: ('Admin' | 'User')[]
  isSubmenu?: boolean
}

export const dashboardNavigation: NavigationItem[] = [
  { name: "Beranda", href: "/dashboard", icon: LayoutDashboard, roles: ['Admin', 'User'] },
  { name: "Perangkat", href: "/dashboard/perangkat", icon: Network, roles: ['Admin'] },
  { name: "Stasiun Benchmark", href: "/dashboard/perangkat-benchmark", icon: Radio, roles: ['Admin'] },
  { name: "Sistem Geografis", href: "/dashboard/peta", icon: Earth, roles: ['Admin', 'User'] },
  { name: "Agrometeorologi", href: "/dashboard/agromet", icon: Sprout, roles: ['Admin', 'User'] },
  { name: "Meteorologi", href: "/dashboard/meteorologi", icon: ChartNoAxesCombined, roles: ['Admin'] },
  { name: "Klimatologis", href: "/dashboard/klimatologi", icon: TrendingUp, roles: ['Admin', 'User'] },
  { name: "Indeks Iklim Global", href: "/dashboard/climate-drivers", icon: Globe, roles: ['Admin', 'User'] },
  { name: "Indeks Monsun", href: "/dashboard/indeks-monsun", icon: Wind, badge: "Regional", roles: ['Admin', 'User'] },
  { name: "Kualitas Udara", href: "/dashboard/air-quality", icon: Wind, badge: "CAMS", roles: ['Admin', 'User'] },
  { name: "Kalibrasi & Validasi", href: "/dashboard/kalibrasi", icon: SlidersHorizontal, roles: ['Admin'] },
  { name: "Basis Data", href: "/dashboard/data", icon: Database, roles: ['Admin'] },
  { name: "Cadangan SQLite", href: "/dashboard/backup-sqlite", icon: Archive, badge: "Offline", roles: ['Admin'] },
  { name: "Laporan Cuaca", href: "/dashboard/laporan", icon: FileText, badge: "Warga", roles: ['Admin'] },
  { name: "Prakirawan Cuaca", href: "/dashboard/prakirawan", icon: CloudRain, roles: ['Admin'] },
  { name: "Profil", href: "/dashboard/profil", icon: User, roles: ['Admin', 'User'] },
  { name: "Manajemen", href: "/dashboard/manager", icon: UsersRound, roles: ['Admin'] },
]
