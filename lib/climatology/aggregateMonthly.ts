// lib/climatology/aggregateMonthly.ts
...
export function aggregateMonthly(rawPoints: SensorDate[]): AggregatedPoint[] {
  const groups = new Map<string, SensorDate[]>();

  for (const r of rawPoints) {
    const parts = getJakartaDateParts(r.timestamp);
    const key = `${parts.year}-${parts.month}`; // e.g. "2026-06"
    if (!groups.has(key)) {
      groups.set(key, []);
    }
  ...
  for (const [ym, items] of groups) {
    ...
    // Monthly rainfall accumulation (positive delta method)
    let rainAccum = 0;
    ...
    const epoch = getJakartaEpoch(yyyy, mm, "01", "00");
