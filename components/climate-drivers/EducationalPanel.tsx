    ...
    <div className="space-y-6">
      ...
      <Card className="border-l-4 border-l-indigo-500 border-none shadow-sm dark:bg-slate-900 bg-white">
        <CardHeader className="pb-2">
          <CardTitle className="text-base sm:text-lg font-extrabold tracking-tight flex items-center gap-2 text-indigo-700 dark:text-indigo-400">
            <Sparkles className="h-5 w-5 text-indigo-500 shrink-0" /> Interpretasi Kondisi Iklim Terkini
          </CardTitle>
      ...
      <Card className="border-none shadow-sm dark:bg-slate-900 bg-white">
        <CardHeader className="pb-3 border-b dark:border-slate-800">
          <CardTitle className="text-base sm:text-lg font-extrabold tracking-tight flex items-center gap-2 text-slate-900 dark:text-slate-100">
            <BookOpen className="h-5 w-5 text-indigo-500 shrink-0" /> Panduan Edukasi &amp; Penjelasan Ilmiah
          </CardTitle>
          <CardDescription className="text-xs text-slate-500 dark:text-slate-400 font-normal">
        ...
        <CardContent className="pt-4">
          <Accordion type="single" collapsible defaultValue="item-1" className="w-full">
            ...
            <AccordionItem value="item-1">
              <AccordionTrigger className="text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 hover:no-underline tracking-tight">
                <span className="flex items-center gap-2 text-left">
                  <HelpCircle className="h-4 w-4 text-indigo-500 shrink-0" />
                  Apa itu {title}?
                </span>
              </AccordionTrigger>
              <AccordionContent className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-normal whitespace-pre-line">
                {whatIsIt}
              </AccordionContent>
            </AccordionItem>
            ...
            {(phaseComparison || (positiveIod && negativeIod)) && (
              <AccordionItem value="item-3">
                ...
                <AccordionContent className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-normal space-y-2">
                  {phaseComparison && (
                    <div className="whitespace-pre-line">{phaseComparison}</div>
                  )}
                  {positiveIod && (
                    <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 rounded-xl text-amber-900 dark:text-amber-200 text-xs sm:text-sm leading-relaxed">
                      {positiveIod}
                    </div>
                  )}
                  {negativeIod && (
                    <div className="p-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 rounded-xl text-blue-900 dark:text-blue-200 text-xs sm:text-sm leading-relaxed">
                      {negativeIod}
                    </div>
                  )}
                </AccordionContent>
                    ...
                    {phaseMeanings.map((p) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-bold font-mono text-slate-900 dark:text-slate-100">Fase {p.phase}</span>
                          <span className="text-slate-500 dark:text-slate-400 font-medium text-[11px]">{p.name}</span>
                        </div>
