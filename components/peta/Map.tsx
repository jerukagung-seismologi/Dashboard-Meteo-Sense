  ...
  const html = `
    <div style="display: flex; flex-direction: column; align-items: center; cursor: pointer;">
      <div style="
        ...
        border: 1.5px solid ${isSelected ? "#818cf8" : isOnline ? "#34d399" : "#94a3b8"};
        background-color: ${isSelected ? "#4338ca" : isOnline ? "#0f172a" : "#334155"};
        color: #ffffff;
      ...
      <div style="
        ...
        border-top: 6px solid ${isSelected ? "#4338ca" : isOnline ? "#0f172a" : "#334155"};
        margin-top: -1px;
      "></div>
    </div>
  `;
...
const Map = ({ devices = [], isDarkMode = false }: MapProps) => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl">
      ...
      <div className="absolute bottom-4 left-4 z-[1000] max-w-[280px] sm:max-w-[340px] p-2.5 sm:p-3 rounded-2xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-lg border border-slate-200/80 dark:border-slate-800 text-xs">
        <div className="flex items-center gap-2">
          <span className="p-1 rounded-lg bg-indigo-500/10 text-indigo-600 dark:text-indigo-400">
            <Radio className="h-4 w-4" />
          </span>
          <div>
            <div className="font-bold text-slate-900 dark:text-slate-100 text-[11px] sm:text-xs">
              Jaringan Stasiun Kebumen ({devices.length} Titik)
            </div>
            <div className="text-[10px] text-slate-500 dark:text-slate-400">
              Klik pin stasiun untuk melihat grafik 1 jam terakhir
            </div>
          </div>
        </div>
      </div>
