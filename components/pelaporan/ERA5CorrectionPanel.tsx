...
export function ERA5CorrectionPanel({
  ...
  // Fetch ERA5 from /api/reanalysis/data
  ...
  return (
    <Card className="border-indigo-100 dark:border-indigo-950/60 bg-gradient-to-br from-indigo-50/30 via-white to-slate-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 shadow-sm overflow-hidden">
      ...
      <CardContent className="p-4 space-y-6">
        {/* --- STAT METRIC CARDS (MBE, RMSE, Pearson r) --- */}
        {biasMetrics ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <strong className={biasMetrics.temperature.mbe > 0 ? "text-red-500" : "text-blue-500"}>
                ...
                <div>
                  ...
                  <strong className="text-slate-700 dark:text-slate-300">{biasMetrics.temperature.rmse}°C</strong>
                </div>
              </div>
              <div className="pt-1 text-[11px] text-slate-500 flex justify-between items-center border-t">
                <span>Rekomendasi Offset:</span>
            ...
            <div className="p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 space-y-1.5">
              <div className="flex justify-between items-center">
                <span className="text-xs font-semibold text-blue-600 flex items-center gap-1">
                  <TrendingUp className="w-3.5 h-3.5" /> Kelembapan Udara
                </span>
                <Badge variant="secondary" className="text-[10px] font-mono">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-slate-400 block text-[10px]">Bias Rata-rata (MBE)</span>
                  <strong className={biasMetrics.humidity.mbe > 0 ? "text-blue-500" : "text-amber-500"}>
                    {biasMetrics.humidity.mbe > 0 ? "+" : ""}{biasMetrics.humidity.mbe}%
                  </strong>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px]">Akurasi (RMSE)</span>
                  <strong className="text-slate-700 dark:text-slate-300">{biasMetrics.humidity.rmse}%</strong>
                </div>
              </div>
              <div className="pt-1 text-[11px] text-slate-500 flex justify-between items-center border-t">
                <span>Rekomendasi Offset:</span>
            ...
            <div className="p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <span className="text-slate-400 block text-[10px]">Akurasi (RMSE)</span>
                  <strong className="text-slate-700 dark:text-slate-300">{biasMetrics.pressure.rmse} hPa</strong>
                </div>
              </div>
              <div className="pt-1 text-[11px] text-slate-500 flex justify-between items-center border-t">
                <span>Rekomendasi Offset:</span>
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-dashed border-slate-200 dark:border-slate-800 text-center text-xs text-slate-500">
            ...
            Klik tombol <strong>"Ambil & Evaluasi ERA5"</strong> di atas untuk memuat data pembanding global dan menghitung deviasi bias sensor.
          </div>
        )}
